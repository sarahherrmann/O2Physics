...
////     }
     ...
     void process(aod::AmbiguousTracks const& tracks, aod::BCs const&, aod::Collisions::iterator const& collisions) //AmbiguousMFTTracks and fwd doesn't work yet
     {
       for (auto& ambitrack : tracks) {
           ...
           //LOGF(info, "  BC %d with global BC %lld", bc.globalIndex(), bc.globalBC());
             ...
             for (auto& collision : collisions)
             {
               ...
               if (collision.bcId() > (bc.globalIndex() - rangeBC) && collision.bcId() < (bc.globalIndex() + rangeBC) )
               {
                   ...
                   if (collision.bcId() == bc.globalIndex() ) registry.fill(HIST("EventSelection"), 3.);
                 ...
                 //printf("T[0] %f, T[1] %f\n", T[0], T[1]);
                   ...
                   SMatrix5 tpars(ambitrack.x(), ambitrack.y(), ambitrack.phi(), ambitrack.tgl(), ambitrack.signed1Pt());

                   std::vector<double> v1{ambitrack.cXX(), ambitrack.cXY(), ambitrack.cYY(), ambitrack.cPhiX(), ambitrack.cPhiY(),
                       ambitrack.cPhiPhi(), ambitrack.cTglX(), ambitrack.cTglY(), ambitrack.cTglPhi(), ambitrack.cTglTgl(),
                       ambitrack.c1PtX(), ambitrack.c1PtY(), ambitrack.c1PtPhi(), ambitrack.c1PtTgl(), ambitrack.c1Pt21Pt2()};

                   SMatrix55 tcovs(v1.begin(), v1.end());

                   o2::track::TrackParCovFwd pars1{ambitrack.z(), tpars, tcovs, ambitrack.chi2()};
